...
#    endif
  int angle;
  Fl_Xlib_Font_Descriptor(const char* xfontname, Fl_Fontsize size, int angle);
#  else
  XUtf8FontStruct* font;        // X UTF-8 font information
  Fl_Xlib_Font_Descriptor(const char* xfontname);
#  if HAVE_GL
  char glok[64];
#  endif // HAVE_GL
#  endif // USE_XFT
  virtual ~Fl_Xlib_Font_Descriptor();
};

//extern Fl_Font_Descriptor *fl_fontsize; // the currently selected one
